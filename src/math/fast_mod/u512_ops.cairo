...
fn u512_add_u256(lhs: u512, rhs: u256) -> u512 {
    ...
    match u256_overflow_add(low, rhs) {
        Result::Ok(u256 { low: limb0, high: limb1 }) => u512 { limb0, limb1, limb2, limb3 },
        Result::Err(limbs) => {
            let u256 { low: limb0, high: limb1 } = limbs;
            // Try to move overflow to limb2
            match u128_overflowing_add(limb2, 1) {
                Result::Ok(limb2) => u512 { limb0, limb1, limb2, limb3 },
                Result::Err(limb2) => {
                    // Try to move overflow to limb3
                    let limb3 = expect_u128(
                        u128_overflowing_add(limb3, 1), 'u512_add_u256 overflow'
                    );
                    u512 { limb0, limb1, limb2, limb3 }
                },
            }
...
fn u512_sub_u256(lhs: u512, rhs: u256) -> u512 {
    ...
    match u256_overflow_sub(low, rhs) {
        ...
        Result::Err(limbs) => {
            let u256 { low: limb0, high: limb1 } = limbs;
            // Try to move overflow to limb2
            match u128_overflowing_sub(limb2, 1) {
                Result::Ok(limb2) => u512 { limb0, limb1, limb2, limb3 },
                Result::Err(limb2) => {
                    // Try to move overflow to limb3
                    let limb3 = expect_u128(
                        u128_overflowing_sub(limb3, 1), 'u512_sub_u256 overflow'
                    );
                    u512 { limb0, limb1, limb2, limb3 }
                },
            }
